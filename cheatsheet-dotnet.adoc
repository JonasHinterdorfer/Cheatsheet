...
In .NET ist Logging über `Microsoft.Extensions.Logging` bereits integriert. Kein zusätzliches NuGet-Paket nötig.
...
* Logger immer über `ILogger<T>` im Konstruktor injizieren (nicht `LoggerFactory` direkt verwenden)
* Log-Level in `appsettings.Development.json` und `appsettings.json` separat konfigurieren
* Structured Logging nutzen: `LogInformation("User {UserId} logged in", userId)` statt String-Interpolation
* Im Development: `Information` oder `Debug`; in Production: `Warning` oder `Error`
